...
Builder.load_file('views/admin_lesson_report_history_view.kv')

class AdminLessonReportHistoryView(Screen):
    def on_load(self, student_metadata):
        ...
        self.ids.subject_label.text = self.metadata["subject_name"]
        self.set_lesson_reports()

    def set_lesson_reports(self):
        reports = self.admin_obj.list_lesson_Reports(self.metadata["student_id"], self.metadata["subject_id"])
        if reports:
