...
from helpers.email_helper import get_grade_set_content, send_mail
from models.student_model import Student
from datetime import datetime
import threading

Builder.load_file('views/admin_add_grades_view.kv')

class AdminAddGradesView(Screen):
    added_grades = []
    ...
    def on_load(self, student_metadata):
        ...
        if "subject_name" in self.student_metadata:
            self.subject_name = self.student_metadata['subject_name']
...
###
    def save_grades(self):
        ...
        todays_entry = self.user_obj.check_grades_date(self.student_metadata["student_id"], curr_date)
        added_sub_ids = []
        if todays_entry:
        ...
        for grade in self.added_grades:
            ...
            if sub_id and sub_id["ID"] in added_sub_ids:
                if grade[0].text != "Select Subject" and grade[2].text != '':
                    self.user_obj.update_grades(self.student_metadata["student_id"], sub_id["ID"], grade[1].text, grade[2].text, curr_date)
                    db_operation = True
            else:
                if grade[0].text != "Select Subject" and grade[2].text != '':
                    self.user_obj.upload_grades(self.student_metadata["student_id"], grade[0].text, grade[1].text, grade[2].text)
                    db_operation = True

        if db_operation:
            self.added_grades = []
            self.ids.grades_grid.clear_widgets()
            mail_dict = get_grade_set_content(self.student_metadata['student_mail'], self.student_metadata['first_name'], self.student_metadata['last_name'])
